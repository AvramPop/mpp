...
public class LabProblemRepository implements SortingRepository<Long, LabProblem>{
  ...
  public Optional<LabProblem> save(LabProblem entity){
    ...
    try{
      if(jdbcOperations.update(query, entity.getId(), entity.getDescription(), entity.getProblemNumber()) == 1){
        return Optional.empty();
      }
  ...
  public Optional<LabProblem> update(LabProblem entity){
    ...
    if(jdbcOperations.update(query,entity.getDescription(),entity.getProblemNumber(),entity.getId()) == 0){
      return Optional.of(entity);
    } else {
      return Optional.empty();
    }
