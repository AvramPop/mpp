...
public class StudentRepository implements SortingRepository<Long, Student>{
  ...
  public Optional<Student> save(Student entity){
    ...
    try{
      if(jdbcOperations.update(query, entity.getId(), entity.getSerialNumber(), entity.getGroup(),entity.getName()) == 1){
        return Optional.empty();
      }
  ...
  public Optional<Student> update(Student entity){
    if (entity == null)
      throw new IllegalArgumentException("Entity must not be null");
    String query = "update public.\"Students\" set serial_number = ?, group_number = ?, name = ? where student_id = ?";
    if(jdbcOperations.update(query,entity.getSerialNumber(),entity.getGroup(),entity.getName(),entity.getId()) == 0){
      return Optional.of(entity);
    } else {
      return Optional.empty();
    }
